...
# Personal OPD (on-policy distillation) script, parallel to train_grpo_v02.sh but with
# do_search=false, OPD teacher RM, and token_reward_direct-style advantages (see README_OPD.md).
# Default data dir matches train_grpo_v02.sh; override DATA_DIR for other parquet trees.
#
# Usage:
#   From repo root (recommended):
#     TEACHER_MODEL=Qwen/Qwen2.5-7B-Instruct ./scripts/experiments/rui_meng/train_opd_v02.sh
#   Or from anywhere:
#     bash /path/to/Search-R1/scripts/experiments/rui_meng/train_opd_v02.sh
#
# Prerequisites:
...
#   CUDA_VISIBLE_DEVICES, DATA_DIR, WAND_PROJECT, BASE_MODEL (student), TEACHER_MODEL, EXPERIMENT_NAME
...
TEACHER_MODEL="${TEACHER_MODEL:-${BASE_MODEL}}"
