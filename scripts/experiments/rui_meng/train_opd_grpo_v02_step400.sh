...
# and token_reward_direct-style advantages (see README_OPD.md). Search is enabled like GRPO:
# starts a retrieval HTTP server reachable at RETRIEVER_URL (default matches v0.2 train_grpo).
# Default data dir matches train_opd_grpo_v02_step400.sh; override DATA_DIR for other parquet trees.
#
# Usage:
#   From repo root (recommended):
#     TEACHER_MODEL=Qwen/Qwen2.5-7B-Instruct ./scripts/experiments/rui_meng/train_opd_grpo_v02_step400.sh
#   Or from anywhere:
#     bash /path/to/Search-R1/scripts/experiments/rui_meng/train_opd_grpo_v02_step400.sh
#
# Prerequisites:
...
#   LOG_PROB_TOP_K, TOP_K_STRATEGY, REWARD_WEIGHT_MODE, TEACHER_TEMPERATURE (rollout teacher temp), N_RESPONSES
...
BASE_MODEL="${BASE_MODEL:-/mnt/task_runtime/Search-R1/verl_checkpoints/verl_checkpoints/nq_search-search-r1-grpo-qwen2.5-3b-em/actor/global_step_400}"
TEACHER_MODEL="${TEACHER_MODEL:-Qwen/Qwen2.5-7B-Instruct}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-nq_opd-grpo-qwen2.5-3b-em-step400}"
